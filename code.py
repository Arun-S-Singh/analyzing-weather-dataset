...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    df[date_col] = df[date_col].apply( lambda x : pd.to_datetime(x).month )
    aggregated_value = df.groupby(date_col)[agg_col].agg(agg)
    #aggregated_value = pd.pivot_table(df,index=date_col,values=agg_col,aggfunc=agg)
...
def group_values(df,col1,agg1):
   ...
   Keyword arguments:
    ...
    #print(weather.groupby('Weather').agg('mean'))
    return grouping
...
def convert(df,celsius):
    ...
    """
    converted_temp = df
    converted_temp = converted_temp[celsius].apply(lambda x : (x*9/5) + 32)
...
#instances_based_condition(weather,'Weather','Cloudy','Visibility (km)',25)
...
print(agg_values_ina_month(weather,'Date/Time','Temp (C)',{'MEAN':'mean','MAX':'max','MIN':'min','SUM':'sum'}))



"""
mb2 = pd.read_excel('', sheetname='Sheet3')
...
mean_weather = group_values(weather,'Weather',{'Temp (C)':'mean','Wind Spd (km/h)':'mean','Dew Point Temp (C)':'mean','Rel Hum (%)':'mean','Wind Spd (km/h)':'mean','Visibility (km)':'mean','Stn Press (kPa)':'mean'})
